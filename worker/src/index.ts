  ...
  try {
    ...
    try {
      try {
        ...
        console.error('Fallback cleanup before anonymize failed', cleanupError);
      }
...
async function handleAdminReplaceDriverStops(
  ...
  try {
    ...
    const stopsPayload: GeocodeSuccess[] = addresses.map((address) => {
      const key = address.trim().toLowerCase();
      const existing = knownByAddress.get(key);
      if (existing && typeof existing.lat === 'number' && typeof existing.lng === 'number') {
        return { address, lat: existing.lat, lng: existing.lng };
      }
      needsGeocode.push(address);
      return { address, lat: DEFAULT_COORDINATE.latitude, lng: DEFAULT_COORDINATE.longitude };
    });
